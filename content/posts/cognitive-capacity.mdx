...
description: "Above a baseline of technique, the player who decides faster and better wins. What the cognitive capacity is, why it becomes the ceiling from early adolescence, and how to train it."
audienceLayer: "Player"
audienceLayerSecondary: "Coach"
category: "Methodology"
codexAnchors:
  convictions: [7, 9, 20, 22, 24, 36]
  capacities:
    primary: "Cognitive"
...
Cognitive capacity, in the StunpreX framework, is the family of mental processes that turn information into action. Where the Perceptual family gathers what the player sees and hears and feels, the Cognitive family decides what to do with it. The two families are inseparable in practice, since a scan without a decision is just a head turn, but they are distinct as development targets, which is why the framework separates them.
...
In Drill 03, **Constrained 1v1 to Score**, Cognitive capacity appears as real-time decision-making under live defensive pressure: which move, which direction, when to shoot, whether to commit to the weak foot. The time window, set at eight seconds in the drill as practice guidance rather than a measured threshold, is not a punishment; it is a cognitive forcing function. Without the time constraint, many players default to waiting for the defender to make a mistake rather than generating a decision of their own. The window closes that exit and demands active choice.
...
## How it appears in real game moments
...
Finally, coaches who provide continuous verbal instruction during reps inadvertently replace cognitive training with external processing. If the coach calls every decision, "left cone, right touch, turn left now", the player does not build internal decision-making. The drill becomes a reaction-to-instruction exercise, which is a useful ability but a different one from decision generation under uncertainty. Constraints do the teaching better: design the environment so that the decision the coach wants is also the decision the environment most clearly demands.
...
**Weekend: Game observation**
One personal leading indicator to track during the game: how often does the player arrive to a receiving moment with a decision already forming? Self-counted, not graded. Compared to their own count next week, not to any other player.
...
*Training must overdo gameplay complexity.* If training is cognitively simpler than the game, the game will feel more cognitively demanding than it needs to be. StunpreX asks training to overdo it deliberately, more decisions, tighter windows, more simultaneous demands, so that the game, when it comes, feels manageable rather than overwhelming.
