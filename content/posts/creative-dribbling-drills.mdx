...
It is a reasonable instinct. But it is incomplete, and in one respect wrong.
...
This distinction matters for drill design. A player with total freedom in training learns to manage total freedom. When the game presents close-marking defenders, a compressed corridor, an opponent's shoulder in the way, and two seconds to make a decision, that player has no preparation for those specific conditions. They solve problems in open space. The game does not offer open space.
...
*Smaller spaces, fewer touches, uneven teams and tight time limits force the player to find new solutions, and they build technique under pressure in a way that comfortable conditions do not. Open space in a training design lets the player avoid adapting..*
...
Each type develops the adaptive capacity from a different angle. Used in combination, they produce the profile the game demands: a player who finds solutions in the specific conditions they've never encountered before, because they've trained the habit of finding solutions in conditions they couldn't fall back on.
...
**Lever 4: Action rule.**
Action rules are a powerful lever, and an underused one. A rule that removes the player's preferred solution (no cut inside, only right foot exit, a two-second pass limit, no return pass, all of them starting points to adjust to the player) forces invention precisely because the default is removed. The player who hates the action rule, who says "that's not how I play", is the player who most needs it. The rule reveals a constraint in their game that the game will expose. Training exposes it first, in a lower-stakes environment where the lesson sticks.
...
3. **Defensive load:** a chasing defender starts about three metres behind the attacker at the moment the ball is received, a starting distance to adjust to the player. Not facing, chasing: the attacker must feel the chase and protect the ball while moving forward.
4. **Action rule:** the attacker must reach the far end of the corridor (about eight metres, as a starting point) without the ball leaving the corridor boundaries. Dribbling out of bounds counts as defender wins.
5. **Objective:** attacker carries to the far end; defender wins by winning possession or forcing out.

****Run five rounds of about thirty seconds each, as practice guidance rather than a prescription. Swap roles.**.**

****Progression** (timings and distances are practice guidance; adjust to the player): after a few sessions, narrow the corridor by about a third. After a few more, add a touch limit: the attacker may not take more than a handful of consecutive touches in the same spot and must keep moving forward. Then add the action rule: the attacker must shield with the body once or more before reaching the end, and if the defender wins possession in the last few metres the attacker loses even on reaching it.).
...
The space changes. The defensive load shifts. The action rule removes the default. The player has to invent: every session. Over time, that invention habit becomes a game habit. The player who has been forced to find a new solution in training two hundred times arrives at the game and finds new solutions there, not because they are naturally creative, but because they have trained the capacity that creativity requires: the ability to adapt under constraint.
...
- [Weak foot dribbling drills: a progression that holds up under pressure](/blog/weak-foot-dribbling-drills): the action rule that removes the preferred foot
